...
# --- CONFIGURATION ---
OUTPUT_CSV = "residue_fragment_atom_counts_withmethod.csv"
...
def get_fragment_smiles(mol2_file):
    smiles = None
    if not os.path.exists(mol2_file) or os.path.getsize(mol2_file) == 0:
        return None
    try:
...
def find_parent_molecule_robust(fragment_smiles, filename):
    if not fragment_smiles: return "-", "-", "No SMILES", "-", 0, "No Search"
    ...
    for attempt in range(3):
        try:
            ...
            try:
                ...
                if hits: 
                    match_method = "Substructure"
                else:
                    match_method = "Substructure (0 Hits)"
            except Exception:
                # STRATEGY B: Similarity Search (Fallback)
                ...
                if hits: 
                    ...
                    match_method = "Similarity (0 Hits)"

            if hits:
                # Parsimony Sort: Prefer smallest parent
                ...
                # Count Parent Atoms Locally (Consistency Check)
                if hasattr(parent, 'smiles') and parent.smiles:
                    parent_atom_count = get_local_atom_count(parent.smiles)
                else:
    ...
    return cid, chebi, name, parent_inchikey, parent_atom_count, match_method
...
def main():
    ...
    headers = [
        ...
        "Fragment_SMILES", "Frag_Heavy_Atoms",  
        "Parent_Heavy_Atoms", "Delta_atom_count", 
        "Parent_CID", "Parent_ChEBI", "Parent_Name", 
        "Match_Method" # <--- NEW COLUMN
    ]
    ...
    with open(OUTPUT_CSV, mode='w', newline='', encoding='utf-8') as csvfile:
        ...
        for idx, f in enumerate(files):
            ...
            if file_hash in HASH_CACHE:
                ...
                # 1. Get raw fragment & count locally
                frag_smi = get_fragment_smiles(f)
                frag_count = get_local_atom_count(frag_smi)
                
                # 2. Find Parent & Method
                cid, chebi, name, p_key, p_count, method = find_parent_molecule_robust(frag_smi, f)
            ...
            # Calculate Delta
            delta = data['p_count'] - data['f_count']

            row = [
                f, res_code, file_hash,
                data['smi'], data['f_count'],
                data['p_count'], delta,
                data['cid'], data['chebi'], data['name'], 
                data['method']
            ]
            writer.writerow(row)
            sys.stdout.flush()
