...
# --- CONFIGURATION ---
LIB_DIR = "/home/user/src/miniforge3/envs/ambertools/dat/leap/lib" # locate where the AMBER slib files are located
...
def run_tleap_generation(lib_path, residues):
    ...
    for res in residues:
            # 1. Save Mol2 (for Amber work)
            mol2_name = f"{filename}_{res}.mol2"
            commands.append(f"saveMol2 {res} {mol2_name} 0")
            
            # 2. Save PDB (For visualization/OpenBabel/ChEBI work)
            pdb_name = f"{filename}_{res}.pdb"
            commands.append(f"savePdb {res} {pdb_name}")
